...
export default function StaffProfile() {
  ...
  const handleSave = async () => {
    try {
      const updated = await staffAPI.update(user._id, formData);
  ...
  const handleChangePassword = async () => {
    ...
    try {
      await staffAPI.changePassword(user._id, passwordData.currentPassword, passwordData.newPassword);
  ...
  return (
    <ProtectedRoute allowedRoles={['teacher', 'admin']}>
      <ModernSidebar menuItems={getStaffMenu(user?.role)} pageTitle="My Profile">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-base font-medium text-gray-500">
                  {user?.subjects?.join(', ') || user?.subject || 'Not assigned'} (Cannot be changed)
                </p>
              </div>
              <div>
